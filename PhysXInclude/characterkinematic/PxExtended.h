...
struct PxExtendedVec3
{
	...
	PX_INLINE void	cross(const PxExtendedVec3& left, const PxVec3& right)
	{
		...
		PxExtended a = (left.y * (double)right.z) - (left.z * (double)right.y);
		PxExtended b = (left.z * (double)right.x) - (left.x * (double)right.z);
		PxExtended c = (left.x * (double)right.y) - (left.y * (double)right.x);
	...
	{
		// temps needed in case left or right is this.
		PxExtended a = ((double)left.y * right.z) - ((double)left.z * right.y);
		PxExtended b = ((double)left.z * right.x) - ((double)left.x * right.z);
		PxExtended c = ((double)left.x * right.y) - ((double)left.y * right.x);
